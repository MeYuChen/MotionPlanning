...
class Node:
    def __init__(self, xind, yind, yawind, direction, x, y,
        ...
        self.xind = xind #格子地图下的横坐标
        self.yind = yind #格子地图下的纵坐标
        self.yawind = yawind #格子地图下的yaw值
        self.direction = direction #朝向
        self.x = x # 真实x坐标
        self.y = y # 真实y坐标
        self.yaw = yaw # 真实yaw
        self.directions = directions # 真实方向
        self.steer = steer 
        self.cost = cost
        self.pind = pind #坐标索引
...
def analystic_expantion(node, ngoal, P):
    ...
    maxc = math.tan(C.MAX_STEER) / C.WB #最小曲率半径
...
def calc_motion_set():
    # 从C.MAX_STEER / C.N_STEER开始直到C.MAX_STEER。步长为C.MAX_STEER / C.N_STEER
...
def calc_parameters(ox, oy, xyreso, yawreso, kdtree):
    ...
    #四舍五入是向上取整，-1.5->-1 -2.5->-2所以minyaw需要-1
...
def main():
    print("start!")
    x, y = 51, 31
    sx, sy, syaw0 = 10.0, 7.0, np.deg2rad(120.0) # 设置起始点 （x,y,heading）实际坐标
    gx, gy, gyaw0 = 45.0, 20.0, np.deg2rad(90.0) # 设置终点 （x,y,heading）实际坐标

    ox, oy = design_obstacles(x, y) # 构建障碍物集合

    t0 = time.time()
    #计算路径
    # 输入 起点，终点，障碍物，长宽分辨率以及角度分辨率
    # 输出 path
